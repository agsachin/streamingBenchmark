...
echo "nohup ${sparkSubmit} --class spark.benchmark.TwitterStreaming --master ${sparkMaster} ${sparkBenchmarkJar}:/opt/install/1-6mod/spark/external/kafka/target/spark-streaming-kafka_2.10-1.6.0-SNAPSHOT.jar ${confFile} > ~/sparkSubmit_${processId}.log 2>&1 &"
`nohup ${sparkSubmit} --class spark.benchmark.TwitterStreaming --master ${sparkMaster} --jars /opt/install/1-6mod/spark/external/kafka/target/spark-streaming-kafka_2.10-1.6.0-SNAPSHOT.jar ${sparkBenchmarkJar} ${confFile} > sparkSubmitx_${processId}.log 2>&1 &`

sleep 10s

echo "" ;echo "" ;echo "**********validating spark submit**********"
echo "cat sparkSubmitx_${processId}.log |tail -100|grep Exception"
tailString=`cat ~/sparkSubmitx_${processId}.log |tail -100`
